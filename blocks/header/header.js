/**
 * Culture Travel — шапка (vanilla JS, без фреймворков)
 * Glass-шапка, якоря, scroll-spy, мобильное меню
 */
...
class SiteHeader {
  ...
  #collectSpyTargets() {
    ...
    // Spy следит за корнями секций (не за заголовками). Заголовки — только точка приземления скролла.
  ...
  #bindEvents() {
    ...
    this.#root.querySelector('.lh__logo')?.addEventListener('click', () => {
      this.closeMenu();
    });
  ...
  #updateActiveLink() {
    ...
    // Линия чуть ниже шапки: иначе при top секции на 80–120px
    // (заголовок уже в кадре) spy ещё держит предыдущий пункт.
    const probe = anchorOffset() + Math.min(140, Math.round(window.innerHeight * 0.18));
    let current = '';

    // Последняя секция, чей верх уже прошёл probe
  ...
  #isHome() {
    if (isHomeDocument()) return true;

    const page = document.documentElement.dataset.page;
    if (page === 'inner') return false;

    if (this.#root.dataset.home === 'true') return true;
    if (this.#root.dataset.home === 'false') return false;
    if (document.body.dataset.page === 'home') return true;

    const path = location.pathname.replace(/\/+$/, '') || '/';
    const base = (document.documentElement.dataset.basePath || '').replace(/\/+$/, '');
    const homePath = base || '/';
    if (path === homePath || path === `${homePath}/index` || /\/(index|home)(\.html)?$/i.test(path)) {
      return Boolean(document.querySelector('.uc-video'));
    }
