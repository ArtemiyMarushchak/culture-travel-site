/**
 * Блок услуг.
 * Десктоп (≥1181px): вкладки-папки с cover-фото.
 * Планшет/мобильный: список → общая модалка услуги с квадратным фото.
 */
...
function servicePayload(pack) {
  const en = langIsEn();
  return {
    index: pack.dataset.serviceIndex || "",
    title: (en && pack.dataset.serviceTitleEn) || pack.dataset.serviceTitle || "",
    text: (en && pack.dataset.serviceTextEn) || pack.dataset.serviceText || "",
    image: pack.dataset.serviceImageMobile || pack.dataset.serviceImage || "",
  };
}
...
export function initServices(root) {
  ...
  function layoutList() {
    ...
    packs.forEach((pack) => {
      pack.classList.remove("is-open", "is-active");
  ...
  function sync() {
    if (desktop.matches) layoutDesktop(current < 0 ? 0 : current);
    else layoutList();
  }
  ...
  tabs.forEach((tab, index) => {
    tab.addEventListener("click", () => {
      ...
      window.openServiceModal?.(servicePayload(packs[index]));
    });
  });
